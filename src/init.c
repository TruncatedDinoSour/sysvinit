...
static CHILD *get_record(FILE *f) {
    ...
    do {
        ...
        switch (cmd = get_cmd(f)) {
            case C_END: get_void(f); return NULL;
            case 0: get_void(f); break;
            case C_REC: break;
            case D_RUNLEVEL:
    ...
    do
        switch (cmd = get_cmd(f)) {
            ...
            case C_EXS:
                ...
                break;
            case C_LEV: get_string(p->rlevel, sizeof(p->rlevel), f); break;
            case C_PROCESS:
...
static void coredump(void) {
    ...
    if (0 != chdir("/"))
        initlog(L_VB, "unable to chdir to /: %s", strerror(errno));
...
#endif
{
    ...
    if ((void *)ctx.eip >= (void *)do_msleep && (void *)ctx.eip < (void *)main)
        p = " (code)";
    initlog(L_VB,
    ...
    while (1) {
        ...
        if ((pid = fork()) == 0) {
            ...
            if (strchr("*#sS", runlevel) && ch->flags & WAITING) {
                 ...
                 * forking off the child or the chld_handler could run before
                 * the waitpid loop has a chance to find its zombie-child.
                 */
                ...
                setsid();
                if ((fd = console_open(O_RDWR | O_NOCTTY)) < 0) {
                    initlog(L_VB, "open(%s): %s", console_dev, strerror(errno));
            ...
            while ((p = strstr(p, "console="))) {
                ...
                if (p != e) {
                    ...
                    if (!dup) {
                        ...
                        sprintf(ch->process,
                                "/sbin/agetty -L -s 115200,38400,9600 %s vt102",
                                dev);
                        ch->next = NULL;
...
static void read_inittab(void) {
    ...
    while (done != 1) {
            ...
            if (tabdir != NULL) {
                if ((file_entry = readdir(tabdir)) != NULL) {
                    ...
                    if (strlen(file_entry->d_name) < 5 ||
                        strcmp(file_entry->d_name + strlen(file_entry->d_name) -
                                   4,
                               ".tab"))
                        continue;
                    /*
        ...
        if (actionNo == -1) {
            initlog(L_VB, "%s[%d]: %s: unknown action field", INITTAB, lineNo,
                    action);
            continue;
        }
        ...
        for (old = newFamily; old; old = old->next) {
            if (strcmp(old->id, id) == 0 && strcmp(id, "~~")) {
                initlog(L_VB, "%s[%d]: duplicate ID field \"%s\"", INITTAB,
                        lineNo, id);
                break;
            }
        }
        ...
        ch->action = actionNo;
        strncpy(ch->id, id,
                sizeof(utproto.ut_id) + 1); /* Hack for different libs. */
    ...
    for (round = 0; round < 2; round++) {
        ...
        for (ch = family; ch; ch = ch->next) {
            ...
            switch (round) {
                case 0: /* Send TERM signal */
                    if (talk)
                        initlog(L_CO, "Sending processes configured via "
        ...
        if (foundOne && round == 0) {
             ...
             *      The f < 100 * sleep_time refers to sleep time in 10
             * millisecond chunks.
             */
    ...
    for (ch = family; ch; ch = ch->next)
        if (ch->flags & KILLME) {
            if (!(ch->flags & ZOMBIE))
                ...
                INITDBG(L_VB, "Updating utmp for pid %d [id %s]", ch->pid,
                        ch->id);
                ch->flags &= ~RUNNING;
                if (ch->process[0] != '+')
...
static int read_level(int arg) {
    ...
    /* Log this action */
    switch (foo) {
        case 'S': initlog(L_VB, "Going single user"); break;
        case 'Q': initlog(L_SY, "Re-reading inittab"); break;
        case 'A':
        case 'B':
        case 'C':
            initlog(L_SY, "Activating demand-procedures for '%c'", foo);
            break;
        case 'U': initlog(L_SY, "Trying to re-exec init"); return 'U';
    }
    if (foo == 'Q') {
    ...
    if (strchr("ABC", foo)) {
        ...
        for (ch = family; ch; ch = ch->next)
            if (strchr(ch->rlevel, foo) != NULL ||
                ...
                ch->flags |= DEMAND;
                ch->flags &= ~XECUTED;
                INITDBG(L_VB, "Marking (%s) as ondemand, flags %d", ch->id,
                        ch->flags);
            }
        return runlevel;
    }
        ...
        while (!quit) {
            ...
            switch (request.cmd) {
                ...
                case INIT_CMD_SETENV:
                    ...
                    break;
                default: initlog(L_VB, "got unimplemented initrequest."); break;
            } /* end of switch */
...
static int telinit(char *progname, int argc, char **argv) {
